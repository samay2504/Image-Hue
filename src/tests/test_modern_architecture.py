...
class TestTransformerEncoder:
    ...
    def test_encoder_freeze_unfreeze(self):
        ...
        frozen_trainable = sum(p.numel() for p in encoder.parameters() if p.requires_grad)
        assert frozen_trainable < initial_trainable
        
        # Unfreeze
        encoder.unfreeze_all()
        unfrozen_trainable = sum(p.numel() for p in encoder.parameters() if p.requires_grad)
...
class TestModernColorizer:
    ...
    def test_colorizer_parameter_counting(self):
        ...
        assert param_counts['total'] == param_counts['encoder'] + param_counts['decoder']
    ...
    def test_colorizer_freeze_unfreeze(self):
        ...
        assert unfrozen_counts['encoder_trainable'] == initial_counts['encoder_trainable']

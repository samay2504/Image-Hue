...
class SPADEDecoder(nn.Module):
    ...
    def __init__(
        ...
        for i, dec_ch in enumerate(decoder_channels):
            ...
            for j in range(num_blocks_per_stage):
                if block_type == "convnext":
                    block = ConvNeXtBlock(dec_ch, label_nc=label_nc, norm_mode=norm_mode)
                else:
                    block = ResidualBlock(dec_ch, label_nc=label_nc, norm_mode=norm_mode)
        ...
        self.upsample_layers = nn.ModuleList([
            nn.Sequential(
                ...
                nn.Conv2d(decoder_channels[i], decoder_channels[i+1], kernel_size=3, padding=1)
            )
            if i < len(decoder_channels) - 1 else nn.Identity()
    ...
    def forward(
        ...
        if x.shape[2:] != L_cond.shape[2:]:
            x = F.interpolate(x, size=L_cond.shape[2:], mode='bilinear', align_corners=False)

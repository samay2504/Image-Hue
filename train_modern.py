...
class ModernColorizationTrainer:
            ...
            if self.global_step % self.config.get("log_interval", 10) == 0:
                ...
                if main_loss_key in losses:
                    self.writer.add_scalar(
                        f"train/{main_loss_key}",
                        losses[main_loss_key].item(),
                        self.global_step,
                    )
                if "perceptual_loss" in losses:
                    self.writer.add_scalar(
    ...
    def validate(self, epoch: int) -> Dict[str, float]:
        ...
        if metrics["perceptual_loss"] > 0:
            self.writer.add_scalar(
                "val/perceptual_loss", metrics["perceptual_loss"], epoch
            )
...
def main():
    ...
    parser.add_argument(
        "--color_stats",
        type=str,
        default="data/color_stats.npz",
        help="Color statistics file",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="base",
        choices=["tiny", "base", "swin"],
        help="Encoder size",
    )
    parser.add_argument(
        "--encoder_model", type=str, default=None, help="Specific HF model name"
    )
    parser.add_argument(
        "--norm_mode",
        type=str,
    ...
    parser.add_argument(
        "--use_checkpointing", action="store_true", help="Enable gradient checkpointing"
    )
    ...
    parser.add_argument(
        "--decoder_lr", type=float, default=1e-4, help="Decoder learning rate"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=10,
        help="Freeze encoder until this epoch",
    )
    parser.add_argument(
        "--use_amp", action="store_true", default=True, help="Use mixed precision"
    )
    ...
    parser.add_argument(
        "--perceptual_weight", type=float, default=0.1, help="Perceptual loss weight"
    )

    # Logging
    parser.add_argument(
        "--log_dir", type=str, default="logs/modern", help="Log directory"
    )
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default="checkpoints/modern",
        help="Checkpoint directory",
    )
    parser.add_argument("--log_interval", type=int, default=10, help="Logging interval")
    parser.add_argument(
        "--save_interval", type=int, default=5, help="Checkpoint save interval"
    )
    parser.add_argument(
        "--keep_last_checkpoints", type=int, default=3, help="Keep last N checkpoints"
    )
    ...
    config["class_weights_path"] = (
        args.color_stats if args.mode == "classification" else None
    )
